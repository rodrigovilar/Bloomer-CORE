...
public class TipoQuestao {
	...
	public static TipoQuestao fromJsonToTipoQuestao(String json) {
		...
		try {
			JsonNode tipoquestaoJSON = objectMapper.readTree(factory
			...
			if (tipoquestaoJSON.has("descricao")) {
				tipoquestao.setDescricao(tipoquestaoJSON.get("descricao")
						.asText());
			}

			if (tipoquestaoJSON.has("tipojogo")) {
				TipoJogo tipojogo = TipoJogo.findTipoJogo(tipoquestaoJSON.get(
						"tipojogo").asLong());
