...
var entity = "jogos"; // O string referente a entidade fica armazenado em uma variável global.
...
function READ(){
	...
	$.getJSON("http://localhost:8080/Bloomer-CORE/"+ entity,
		function(json){
			...
			var header = '<tr><th>ID</th><th>Nome</th><th>ID do Tipo Jogo</th><th>ID do Configurador</th><th>Edit</th><th>Delete</th></tr>';
			table.append(header);

			for (i=0; i < json.length; i++){
			    var row = '<tr><td>' + json[i].id + '</td><td>' + json[i].nome + '</td><td>' + json[i].tipojogo +
			    		  '</td><td>' + json[i].configurador + '</td><td><a href="#" onclick="fillForm(' + json[i].id +
...
function buildJSON(){
	...
	'", "configurador":"'+ $('input[name="configurador"]').val() + '"}';
...
function buildForm(type){
	...
	var form = $('<form action="'+action+'"><br>' +
					 ...
					 '<input name="configurador" type="text" placeholder="ID do Configurador" /><br>' +
...
function fillForm(id){
	...
	$.getJSON("http://localhost:8080/Bloomer-CORE/" +entity+ "/" +id,
		...
		function(json){
			...
			$("input[name='configurador']").val(json.configurador);
